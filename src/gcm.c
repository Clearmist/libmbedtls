...
int mbedtls_gcm_setkey(
    mbedtls_gcm_context *ctx, mbedtls_cipher_id_t cipher, const unsigned char *key, unsigned int keybits)
{
    int ret;
    ...
    unsigned char *output)
{
    ...
    if ((ret = mbedtls_gcm_crypt_and_tag(
             ctx, MBEDTLS_GCM_DECRYPT, length, iv, iv_len, add, add_len, input, output, tag_len, check_tag)) != 0)
    {
        return (ret);
    }
    ...
    for (j = 0; j < 3; j++)
    {
        ...
        for (i = 0; i < MAX_TESTS; i++)
        {
            ...
            ret = mbedtls_gcm_crypt_and_tag(
                &ctx, MBEDTLS_GCM_ENCRYPT, pt_len[i], iv[iv_index[i]], iv_len[i], additional[add_index[i]], add_len[i],
            ...
            ret = mbedtls_gcm_crypt_and_tag(
                &ctx, MBEDTLS_GCM_DECRYPT, pt_len[i], iv[iv_index[i]], iv_len[i], additional[add_index[i]], add_len[i],
            ...
            ret = mbedtls_gcm_starts(
                &ctx, MBEDTLS_GCM_ENCRYPT, iv[iv_index[i]], iv_len[i], additional[add_index[i]], add_len[i]);
            if (ret != 0)
                goto exit;
            ...
            ret = mbedtls_gcm_starts(
                &ctx, MBEDTLS_GCM_DECRYPT, iv[iv_index[i]], iv_len[i], additional[add_index[i]], add_len[i]);
            if (ret != 0)
                goto exit;
