...
static int x509_get_uid(unsigned char **p, const unsigned char *end, mbedtls_x509_buf *uid, int n)
{
    ...
    if ((ret = mbedtls_asn1_get_tag(p, end, &uid->len, MBEDTLS_ASN1_CONTEXT_SPECIFIC | MBEDTLS_ASN1_CONSTRUCTED | n)) !=
        0)
    {
        if (ret == MBEDTLS_ERR_ASN1_UNEXPECTED_TAG)
            return (0);
 ...
 */
static int
x509_get_subject_alt_name(unsigned char **p, const unsigned char *end, mbedtls_x509_sequence *subject_alt_name)
{
    int ret;
    size_t len, tag_len;
    ...
    if (buf_format == MBEDTLS_X509_FORMAT_PEM)
    {
        ...
        while (buflen > 1)
        {
            ...
            ret = mbedtls_pem_read_buffer(
                &pem, "-----BEGIN CERTIFICATE-----", "-----END CERTIFICATE-----", buf, NULL, 0, &use_len);
...
int mbedtls_x509_crt_parse_path(mbedtls_x509_crt *chain, const char *path)
{
    ...
    do
    {
        ...
        w_ret = WideCharToMultiByte(
            CP_ACP, 0, file_data.cFileName, lstrlenW(file_data.cFileName), p, (int)len - 1, NULL, NULL);
        if (w_ret == 0)
        {
...
int mbedtls_x509_crt_info(char *buf, size_t size, const char *prefix, const mbedtls_x509_crt *crt)
{
    ...
    ret = mbedtls_snprintf(
        ...
        prefix, crt->valid_from.year, crt->valid_from.mon, crt->valid_from.day, crt->valid_from.hour,
    ...
    ret = mbedtls_snprintf(
        ...
        "\n%sexpires on        : "
        "%04d-%02d-%02d %02d:%02d:%02d",
        prefix, crt->valid_to.year, crt->valid_to.mon, crt->valid_to.day, crt->valid_to.hour, crt->valid_to.min,
        crt->valid_to.sec);
    MBEDTLS_X509_SAFE_SNPRINTF;
 ...
 */
static int x509_crt_verifycrl(
    mbedtls_x509_crt *crt, mbedtls_x509_crt *ca, mbedtls_x509_crl *crl_list, const mbedtls_x509_crt_profile *profile)
{
    int flags = 0;
    ...
    while (crl_list != NULL)
    {
        ...
        if (mbedtls_pk_verify_ext(
                crl_list->sig_pk, crl_list->sig_opts, &ca->pk, crl_list->sig_md, hash, mbedtls_md_get_size(md_info),
 ...
 */
static int
x509_crt_check_signature(const mbedtls_x509_crt *child, mbedtls_x509_crt *parent, mbedtls_x509_crt_restart_ctx *rs_ctx)
{
    const mbedtls_md_info_t *md_info;
    unsigned char hash[MBEDTLS_MD_MAX_SIZE];
    ...
    if (rs_ctx != NULL && child->sig_pk == MBEDTLS_PK_ECDSA)
    {
        return (mbedtls_pk_verify_restartable(
            &parent->pk, child->sig_md, hash, mbedtls_md_get_size(md_info), child->sig.p, child->sig.len, &rs_ctx->pk));
    }
#else
    (void)rs_ctx;
#endif
    ...
    return (mbedtls_pk_verify_ext(
        child->sig_pk, child->sig_opts, &parent->pk, child->sig_md, hash, mbedtls_md_get_size(md_info), child->sig.p,
        child->sig.len));
}
    ...
    mbedtls_x509_crt_restart_ctx *rs_ctx)
{
    ...
    *parent_is_trusted = 1;
    ...
    while (1)
    {
        ...
        ret = x509_crt_find_parent_in(
            child, search_list, parent, signature_is_good, *parent_is_trusted, path_cnt, self_cnt, rs_ctx);
    ...
    mbedtls_x509_crt_restart_ctx *rs_ctx)
{
    ...
    find_parent:
        ...
        ret = x509_crt_find_parent(
            child, trust_ca, &parent, &parent_is_trusted, &signature_is_good, ver_chain->len - 1, self_cnt, rs_ctx);
...
int mbedtls_x509_crt_verify(
    ...
    void *p_vrfy)
{
    return (mbedtls_x509_crt_verify_restartable(
        crt, trust_ca, ca_crl, &mbedtls_x509_crt_profile_default, cn, flags, f_vrfy, p_vrfy, NULL));
}
...
int mbedtls_x509_crt_verify_with_profile(
    ...
    void *p_vrfy)
{
    return (mbedtls_x509_crt_verify_restartable(crt, trust_ca, ca_crl, profile, cn, flags, f_vrfy, p_vrfy, NULL));
}
