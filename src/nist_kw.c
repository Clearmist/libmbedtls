    ...
    const int is_wrap)
{
    ...
    if ((ret = mbedtls_cipher_setkey(&ctx->cipher_ctx, key, keybits, is_wrap ? MBEDTLS_ENCRYPT : MBEDTLS_DECRYPT)) != 0)
    {
        return (ret);
    }
...
int mbedtls_nist_kw_self_test(int verbose)
{
    ...
    for (i = 0; i < KW_TESTS; i++)
    {
        ...
        ret = mbedtls_nist_kw_wrap(&ctx, MBEDTLS_KW_MODE_KWP, kwp_msg[i], kwp_msg_len[i], out, &olen, sizeof(out));
