...
#define CHACHAPOLY_VALIDATE_RET(cond) MBEDTLS_INTERNAL_VALIDATE_RET(cond, MBEDTLS_ERR_POLY1305_BAD_INPUT_DATA)
...
int mbedtls_chachapoly_starts(
    mbedtls_chachapoly_context *ctx, const unsigned char nonce[12], mbedtls_chachapoly_mode_t mode)
{
    int ret;
    ...
    ret = mbedtls_chacha20_update(&ctx->chacha20_ctx, sizeof(poly1305_key), poly1305_key, poly1305_key);
    if (ret != 0)
        goto cleanup;
...
int mbedtls_chachapoly_update(
    mbedtls_chachapoly_context *ctx, size_t len, const unsigned char *input, unsigned char *output)
{
    int ret;
    CHACHAPOLY_VALIDATE_RET(ctx != NULL);
    ...
    unsigned char tag[16])
{
    ...
    return (chachapoly_crypt_and_tag(ctx, MBEDTLS_CHACHAPOLY_ENCRYPT, length, nonce, aad, aad_len, input, output, tag));
}
    ...
    unsigned char *output)
{
    ...
    if ((ret = chachapoly_crypt_and_tag(
             ctx, MBEDTLS_CHACHAPOLY_DECRYPT, length, nonce, aad, aad_len, input, output, check_tag)) != 0)
    {
        return (ret);
    }
...
static const unsigned char test_key[1][32] = {{0x80, 0x81, 0x82, 0x83, 0x84, 0x85, 0x86, 0x87, 0x88, 0x89, 0x8a,
...
static const unsigned char test_aad[1][12] = {{0x50, 0x51, 0x52, 0x53, 0xc0, 0xc1, 0xc2, 0xc3, 0xc4, 0xc5, 0xc6, 0xc7}};
...
int mbedtls_chachapoly_self_test(int verbose)
{
    ...
    for (i = 0U; i < 1U; i++)
    {
        ...
        ret = mbedtls_chachapoly_encrypt_and_tag(
            &ctx, test_input_len[i], test_nonce[i], test_aad[i], test_aad_len[i], test_input[i], output, mac);
