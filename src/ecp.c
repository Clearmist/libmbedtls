...
int mbedtls_ecp_point_read_binary(
    const mbedtls_ecp_group *grp, mbedtls_ecp_point *pt, const unsigned char *buf, size_t ilen)
{
    int ret;
    size_t plen;
    ECP_VALIDATE_RET(grp != NULL);
...
int mbedtls_ecp_tls_read_point(
    const mbedtls_ecp_group *grp, mbedtls_ecp_point *pt, const unsigned char **buf, size_t buf_len)
{
...
static int ecp_add_mixed(
    const mbedtls_ecp_group *grp, mbedtls_ecp_point *R, const mbedtls_ecp_point *P, const mbedtls_ecp_point *Q)
{
    int ret;
 ...
 */
static int ecp_randomize_jac(
    const mbedtls_ecp_group *grp, mbedtls_ecp_point *pt, int (*f_rng)(void *, unsigned char *, size_t), void *p_rng)
{
    int ret;
    mbedtls_mpi l, ll;
    size_t p_size;
    ...
    } while ((mbedtls_mpi_cmp_int(&l, 1) <= 0) || (mbedtls_mpi_cmp_mpi(&l, &grp->P) >= 0));
 ...
 * Cost: 2M
 */
static int ecp_randomize_mxz(
    const mbedtls_ecp_group *grp, mbedtls_ecp_point *P, int (*f_rng)(void *, unsigned char *, size_t), void *p_rng)
{
    int ret;
    mbedtls_mpi l;
    size_t p_size;
...
int mbedtls_ecp_gen_privkey(
    const mbedtls_ecp_group *grp, mbedtls_mpi *d, int (*f_rng)(void *, unsigned char *, size_t), void *p_rng)
{
    int ret = MBEDTLS_ERR_ECP_BAD_INPUT_DATA;
    size_t n_size;
#if defined(ECP_SHORTWEIERSTRASS)
...
int mbedtls_ecp_gen_key(
    mbedtls_ecp_group_id grp_id, mbedtls_ecp_keypair *key, int (*f_rng)(void *, unsigned char *, size_t), void *p_rng)
{
    int ret;
...
static const uint8_t test_kdf_out[40] = {
    0x3e, 0xf6, 0xda, 0xf9, 0x51, 0x60, 0x70, 0x5f, 0xdf, 0x21, 0xcd, 0xab, 0xac, 0x25,
    0x7b, 0x05, 0xfe, 0xc1, 0xab, 0x7c, 0xc9, 0x68, 0x43, 0x25, 0x8a, 0xfc, 0x40, 0x6e,
    0x5b, 0xf7, 0x98, 0x27, 0x10, 0xfa, 0x7b, 0x93, 0x52, 0xd4, 0x16, 0xaa,
};
...
static const uint8_t test_kdf_out[40] = {
    0x7d, 0xf6, 0x41, 0xf8, 0x3c, 0x47, 0xdc, 0x28, 0x5f, 0x7f, 0xaa, 0xde, 0x05, 0x64,
    0xd6, 0x25, 0x00, 0x6a, 0x47, 0xd9, 0x1e, 0xa4, 0xa0, 0x8c, 0xd7, 0xf7, 0x0c, 0x99,
    0xaa, 0xa0, 0x72, 0x66, 0x69, 0x0e, 0x25, 0xaa, 0xa1, 0x63, 0x14, 0x79,
};
