...
int mbedtls_ssl_derive_keys(mbedtls_ssl_context *ssl)
{
    ...
    if (handshake->resume == 0)
    {
        ...
        if (ssl->handshake->extended_ms == MBEDTLS_SSL_EXTENDED_MS_ENABLED)
        {
            ...
            ret = handshake->tls_prf(
                handshake->premaster, handshake->pmslen, "extended master secret", session_hash, hash_len,
                session->master, 48);
            if (ret != 0)
            {
...
#endif
            ret = handshake->tls_prf(
                handshake->premaster, handshake->pmslen, "master secret", handshake->randbytes, 64, session->master,
                48);
        if (ret != 0)
        {
            MBEDTLS_SSL_DEBUG_RET(1, "prf", ret);
    ...
    MBEDTLS_SSL_DEBUG_MSG(
        3, ("keylen: %d, minlen: %d, ivlen: %d, maclen: %d", transform->keylen, transform->minlen, transform->ivlen,
            transform->maclen));
    ...
    if (mbedtls_ssl_hw_record_init != NULL)
    {
        ...
        if ((ret = mbedtls_ssl_hw_record_init(
                 ssl, key1, key2, transform->keylen, transform->iv_enc, transform->iv_dec, iv_copy_len, mac_enc,
    ...
    if (ssl->conf->f_export_keys != NULL)
    {
        ssl->conf->f_export_keys(
            ssl->conf->p_export_keys, session->master, keyblk, mac_key_len, transform->keylen, iv_copy_len);
    }
#endif
    ...
    if (session->compression == MBEDTLS_SSL_COMPRESS_DEFLATE)
    {
        if (ssl->compress_buf == NULL)
        {
            ...
            if (ssl->compress_buf == NULL)
            {
                MBEDTLS_SSL_DEBUG_MSG(1, ("alloc(%d bytes) failed", MBEDTLS_SSL_COMPRESS_BUFFER_LEN));
                return (MBEDTLS_ERR_SSL_ALLOC_FAILED);
            }
        }
...
int mbedtls_ssl_psk_derive_premaster(mbedtls_ssl_context *ssl, mbedtls_key_exchange_type_t key_ex)
{
        ...
        if (key_ex == MBEDTLS_KEY_EXCHANGE_DHE_PSK)
    {
        ...
        if ((ret = mbedtls_dhm_calc_secret(
                 &ssl->handshake->dhm_ctx, p + 2, end - (p + 2), &len, ssl->conf->f_rng, ssl->conf->p_rng)) != 0)
        {
        ...
        if (key_ex == MBEDTLS_KEY_EXCHANGE_ECDHE_PSK)
    {
        ...
        if ((ret = mbedtls_ecdh_calc_secret(
                 &ssl->handshake->ecdh_ctx, &zlen, p + 2, end - (p + 2), ssl->conf->f_rng, ssl->conf->p_rng)) != 0)
        {
...
static int ssl_encrypt_buf(mbedtls_ssl_context *ssl)
{
                                        ...
                                        ))
    {
        ...
        if (ssl->minor_ver == MBEDTLS_SSL_MINOR_VERSION_0)
        {
            ...
            ret = ssl_mac(
                &ssl->transform_out->md_ctx_enc, ssl->transform_out->mac_enc, ssl->out_msg, ssl->out_msglen,
                ssl->out_ctr, ssl->out_msgtype, mac);
            ...
            if (ssl->minor_ver >= MBEDTLS_SSL_MINOR_VERSION_1)
        {
            ...
            if (ret != 0)
                goto hmac_failed_etm_disabled;
            ret = mbedtls_md_hmac_update(&ssl->transform_out->md_ctx_enc, ssl->out_msg, ssl->out_msglen);
    ...
    if (mode == MBEDTLS_MODE_STREAM)
    {
        ...
        if ((ret = mbedtls_cipher_crypt(
                 &ssl->transform_out->cipher_ctx_enc, ssl->transform_out->iv_enc, ssl->transform_out->ivlen,
                 ssl->out_msg, ssl->out_msglen, ssl->out_msg, &olen)) != 0)
        {
        ...
        if (mode == MBEDTLS_MODE_CBC)
    {
        ...
        if ((ret = mbedtls_cipher_crypt(
                 &ssl->transform_out->cipher_ctx_enc, ssl->transform_out->iv_enc, ssl->transform_out->ivlen, enc_msg,
                 enc_msglen, enc_msg, &olen)) != 0)
        {
            MBEDTLS_SSL_DEBUG_RET(1, "mbedtls_cipher_crypt", ret);
            return (ret);
        }
        ...
        if (auth_done == 0)
        {
            ...
            ret = mbedtls_md_hmac_update(&ssl->transform_out->md_ctx_enc, ssl->out_iv, ssl->out_msglen);
            if (ret != 0)
                goto hmac_failed_etm_enabled;
            ret = mbedtls_md_hmac_finish(&ssl->transform_out->md_ctx_enc, mac);
            if (ret != 0)
...
static int ssl_decrypt_buf(mbedtls_ssl_context *ssl)
{
    ...
    if (mode == MBEDTLS_MODE_STREAM)
    {
        ...
        if ((ret = mbedtls_cipher_crypt(
                 &ssl->transform_in->cipher_ctx_dec, ssl->transform_in->iv_dec, ssl->transform_in->ivlen, ssl->in_msg,
                 ssl->in_msglen, ssl->in_msg, &olen)) != 0)
        {
            MBEDTLS_SSL_DEBUG_RET(1, "mbedtls_cipher_crypt", ret);
            return (ret);
        }
        ...
        if (mode == MBEDTLS_MODE_GCM || mode == MBEDTLS_MODE_CCM || mode == MBEDTLS_MODE_CHACHAPOLY)
    {
        ...
        if ((ret = mbedtls_cipher_auth_decrypt(
                 &ssl->transform_in->cipher_ctx_dec, iv, transform->ivlen, add_data, 13, dec_msg, dec_msglen,
                 dec_msg_result, &olen, dec_msg + dec_msglen, taglen)) != 0)
        {
        ...
        if (mode == MBEDTLS_MODE_CBC)
    {
        ...
        if (ssl->session_in->encrypt_then_mac == MBEDTLS_SSL_ETM_ENABLED)
        {
            ...
            if (ret != 0)
                goto hmac_failed_etm_enabled;
            ret = mbedtls_md_hmac_update(&ssl->transform_in->md_ctx_dec, ssl->in_iv, ssl->in_msglen);
            if (ret != 0)
                goto hmac_failed_etm_enabled;
        ...
        if ((ret = mbedtls_cipher_crypt(
                 &ssl->transform_in->cipher_ctx_dec, ssl->transform_in->iv_dec, ssl->transform_in->ivlen, dec_msg,
                 dec_msglen, dec_msg_result, &olen)) != 0)
        {
            MBEDTLS_SSL_DEBUG_RET(1, "mbedtls_cipher_crypt", ret);
            return (ret);
        }
        ...
        if (ssl->in_msglen < ssl->transform_in->maclen + padlen && auth_done == 0)
        {
            ...
            MBEDTLS_SSL_DEBUG_MSG(
                1, ("msglen (%d) < maclen (%d) + padlen (%d)", ssl->in_msglen, ssl->transform_in->maclen, padlen));
#endif
            padlen = 0;
            correct = 0;
        }
    ...
    if (auth_done == 0)
    {
        ...
        {
            ret = ssl_mac(
                &ssl->transform_in->md_ctx_dec, ssl->transform_in->mac_dec, ssl->in_msg, ssl->in_msglen, ssl->in_ctr,
                ssl->in_msgtype, mac_expect);
            if (ret != 0)
            {
                MBEDTLS_SSL_DEBUG_RET(1, "ssl_mac", ret);
                return (ret);
            ...
            if (ssl->minor_ver > MBEDTLS_SSL_MINOR_VERSION_0)
        {
            ...
            ret = mbedtls_ssl_cf_hmac(
                &ssl->transform_in->md_ctx_dec, add_data, sizeof(add_data), ssl->in_msg, ssl->in_msglen, min_len,
                max_len, mac_expect);
            if (ret != 0)
            {
            ...
            mbedtls_ssl_cf_memcpy_offset(
                mac_peer, ssl->in_msg, ssl->in_msglen, min_len, max_len, ssl->transform_in->maclen);
        }
        else
#endif /* MBEDTLS_SSL_PROTO_TLS1 || MBEDTLS_SSL_PROTO_TLS1_1 ||                                                        \
              MBEDTLS_SSL_PROTO_TLS1_2 */
...
static int ssl_compress_buf(mbedtls_ssl_context *ssl)
{
    ...
    ssl->out_msglen = MBEDTLS_SSL_OUT_BUFFER_LEN - ssl->transform_out->ctx_deflate.avail_out - bytes_written;
...
static int ssl_decompress_buf(mbedtls_ssl_context *ssl)
{
    ...
    ssl->transform_in->ctx_inflate.avail_out = MBEDTLS_SSL_IN_BUFFER_LEN - header_bytes;
    ...
    ssl->in_msglen = MBEDTLS_SSL_IN_BUFFER_LEN - ssl->transform_in->ctx_inflate.avail_out - header_bytes;
...
int mbedtls_ssl_fetch_input(mbedtls_ssl_context *ssl, size_t nb_want)
{
    ...
    if (ssl->conf->transport == MBEDTLS_SSL_TRANSPORT_DATAGRAM)
    {
        ...
        if (ret == MBEDTLS_ERR_SSL_TIMEOUT)
        {
            ...
            if (ssl->state != MBEDTLS_SSL_HANDSHAKE_OVER)
            {
                ...
                ssl->conf->endpoint == MBEDTLS_SSL_IS_SERVER && ssl->renego_status == MBEDTLS_SSL_RENEGOTIATION_PENDING)
            {
...
#endif
    {
        ...
        while (ssl->in_left < nb_want)
        {
            ...
            {
                MBEDTLS_SSL_DEBUG_MSG(
                    1, ("f_recv returned %d bytes but only %lu were requested", ret, (unsigned long)len));
                return (MBEDTLS_ERR_SSL_INTERNAL_ERROR);
            }
...
int mbedtls_ssl_flush_output(mbedtls_ssl_context *ssl)
{
    ...
    {
        MBEDTLS_SSL_DEBUG_MSG(
            2, ("message length: %d, out_left: %d", mbedtls_ssl_hdr_len(ssl) + ssl->out_msglen, ssl->out_left));
        ...
        {
            MBEDTLS_SSL_DEBUG_MSG(
                1, ("f_send returned %d bytes but only %lu bytes were sent", ret, (unsigned long)ssl->out_left));
            return (MBEDTLS_ERR_SSL_INTERNAL_ERROR);
        }
    ...
    while (ssl->handshake->cur_msg != NULL)
    {
        ...
        else
        {
            ...
            if (frag_off == 0 && cur_hs_frag_len != hs_len)
            {
                MBEDTLS_SSL_DEBUG_MSG(
                    2,
                    ("fragmenting handshake message (%u > %u)", (unsigned)cur_hs_frag_len, (unsigned)max_hs_frag_len));
            }
...
int mbedtls_ssl_write_handshake_msg(mbedtls_ssl_context *ssl)
{
    ...
    if (!(ssl->out_msgtype == MBEDTLS_SSL_MSG_HANDSHAKE && hs_type == MBEDTLS_SSL_HS_HELLO_REQUEST) &&
        ssl->handshake == NULL)
    {
        MBEDTLS_SSL_DEBUG_MSG(1, ("should never happen"));
        return (MBEDTLS_ERR_SSL_INTERNAL_ERROR);
...
int mbedtls_ssl_prepare_handshake_record(mbedtls_ssl_context *ssl)
{
    ...
    if (ssl->conf->transport == MBEDTLS_SSL_TRANSPORT_DATAGRAM)
    {
             ...
             (ssl->state == MBEDTLS_SSL_HANDSHAKE_OVER && ssl->in_msg[0] != MBEDTLS_SSL_HS_CLIENT_HELLO)))
        {
            ...
            {
                MBEDTLS_SSL_DEBUG_MSG(
                    2, ("received future handshake message of sequence number %u (next %u)", recv_msg_seq,
...
{
    return (
        ((uint64_t)buf[0] << 40) | ((uint64_t)buf[1] << 32) | ((uint64_t)buf[2] << 24) | ((uint64_t)buf[3] << 16) |
    ...
    size_t *olen)
{
     ...
     */
    if (in_len < 61 || in[0] != MBEDTLS_SSL_MSG_HANDSHAKE || in[3] != 0 || in[4] != 0 || in[19] != 0 || in[20] != 0 ||
        in[21] != 0)
    {
...
static int ssl_handle_possible_reconnect(mbedtls_ssl_context *ssl)
{
    ...
    ret = ssl_check_dtls_clihlo_cookie(
        ssl->conf->f_cookie_write, ssl->conf->f_cookie_check, ssl->conf->p_cookie, ssl->cli_id, ssl->cli_id_len,
        ssl->in_buf, ssl->in_left, ssl->out_buf, MBEDTLS_SSL_OUT_CONTENT_LEN, &len);
...
static int ssl_parse_record_header(mbedtls_ssl_context *ssl)
{
        ...
        ssl->in_msgtype != MBEDTLS_SSL_MSG_CHANGE_CIPHER_SPEC && ssl->in_msgtype != MBEDTLS_SSL_MSG_APPLICATION_DATA)
    {
        ...
        if (ssl->conf->transport != MBEDTLS_SSL_TRANSPORT_DATAGRAM)
            ...
            mbedtls_ssl_send_alert_message(
                ssl, MBEDTLS_SSL_ALERT_LEVEL_FATAL, MBEDTLS_SSL_ALERT_MSG_UNEXPECTED_MESSAGE);
    ...
    if (ssl->conf->transport == MBEDTLS_SSL_TRANSPORT_DATAGRAM)
    {
        ...
        if (rec_epoch != ssl->in_epoch)
        {
             ...
             */
            if (ssl->conf->endpoint == MBEDTLS_SSL_IS_SERVER && ssl->state == MBEDTLS_SSL_HANDSHAKE_OVER &&
                rec_epoch == 0 && ssl->in_msgtype == MBEDTLS_SSL_MSG_HANDSHAKE && ssl->in_left > 13 &&
    ...
    else
    {
         ...
         */
        if (ssl->minor_ver >= MBEDTLS_SSL_MINOR_VERSION_1 &&
            ssl->in_msglen > ssl->transform_in->minlen + MBEDTLS_SSL_IN_CONTENT_LEN + 256)
        {
            MBEDTLS_SSL_DEBUG_MSG(1, ("bad message length"));
            return (MBEDTLS_ERR_SSL_INVALID_RECORD);
        }
...
static int ssl_load_buffered_message(mbedtls_ssl_context *ssl)
{
    ...
    /* Debug only */
    {
        ...
        for (offset = 1; offset < MBEDTLS_SSL_MAX_BUFFERED_HS; offset++)
        {
            ...
            {
                MBEDTLS_SSL_DEBUG_MSG(
                    2, ("Future message with sequence number %u %s buffered.", hs->in_msg_seq + offset,
...
static int ssl_buffer_make_space(mbedtls_ssl_context *ssl, size_t desired)
{
    ...
    {
        MBEDTLS_SSL_DEBUG_MSG(
            2, ("Free buffering slot %d to make space for reassembly of next handshake message", offset));
...
static int ssl_buffer_message(mbedtls_ssl_context *ssl)
{
    ...
    case MBEDTLS_SSL_MSG_HANDSHAKE:
    {
        ...
        MBEDTLS_SSL_DEBUG_MSG(
            2, ("Buffering HS message with sequence number %u, offset %u ", recv_msg_seq, recv_msg_seq_offset));
        ...
        if (!hs_buf->is_valid)
        {
            ...
            if (reassembly_buf_sz > (MBEDTLS_SSL_DTLS_MAX_BUFFERING - hs->buffering.total_bytes_buffered))
            {
                if (recv_msg_seq_offset > 0)
                {
                    ...
                    MBEDTLS_SSL_DEBUG_MSG(
                        2, ("Buffering of future message of size %u would exceed the compile-time limit %u (already %u "
                ...
                {
                    MBEDTLS_SSL_DEBUG_MSG(
                        2, ("Reassembly of next message of size %u (%u with bitmap) would exceed the compile-time "
                            "limit %u (already %u bytes buffered) -- fail\n",
...
static int ssl_buffer_future_record(mbedtls_ssl_context *ssl)
{
    ...
    {
        MBEDTLS_SSL_DEBUG_MSG(
            2, ("Buffering of future epoch record of size %u would exceed the compile-time limit %u (already %u bytes "
                "buffered) -- ignore\n",
        ...
        if (ssl->conf->transport == MBEDTLS_SSL_TRANSPORT_DATAGRAM)
        {
            ...
            if (ret == MBEDTLS_ERR_SSL_INVALID_RECORD || ret == MBEDTLS_ERR_SSL_INVALID_MAC)
            {
                ...
                if (ssl->state == MBEDTLS_SSL_CLIENT_FINISHED || ssl->state == MBEDTLS_SSL_SERVER_FINISHED)
                {
                    ...
                    {
                        mbedtls_ssl_send_alert_message(
                            ssl, MBEDTLS_SSL_ALERT_LEVEL_FATAL, MBEDTLS_SSL_ALERT_MSG_BAD_RECORD_MAC);
                    }
#endif
                    return (ret);
                }
...
#if defined(MBEDTLS_SSL_PROTO_DTLS)
        if (ssl->conf->transport == MBEDTLS_SSL_TRANSPORT_DATAGRAM &&
            ssl->state != MBEDTLS_SSL_CLIENT_CHANGE_CIPHER_SPEC && ssl->state != MBEDTLS_SSL_SERVER_CHANGE_CIPHER_SPEC)
        {
            if (ssl->handshake == NULL)
            {
...
int mbedtls_ssl_send_fatal_handshake_failure(mbedtls_ssl_context *ssl)
{
    ...
    if ((ret = mbedtls_ssl_send_alert_message(
             ssl, MBEDTLS_SSL_ALERT_LEVEL_FATAL, MBEDTLS_SSL_ALERT_MSG_HANDSHAKE_FAILURE)) != 0)
    {
        return (ret);
    }
 ...
 */
    ...
    !defined(MBEDTLS_KEY_EXCHANGE_DHE_RSA_ENABLED) && !defined(MBEDTLS_KEY_EXCHANGE_ECDHE_RSA_ENABLED) &&              \
...
static int ssl_parse_certificate_chain(mbedtls_ssl_context *ssl)
{
    ...
    {
        if (ssl->in_msglen == 2 && ssl->in_msgtype == MBEDTLS_SSL_MSG_ALERT &&
            ssl->in_msg[0] == MBEDTLS_SSL_ALERT_LEVEL_WARNING && ssl->in_msg[1] == MBEDTLS_SSL_ALERT_MSG_NO_CERT)
        {
    ...
    if (ssl->conf->endpoint == MBEDTLS_SSL_IS_CLIENT && ssl->renego_status == MBEDTLS_SSL_RENEGOTIATION_IN_PROGRESS)
    {
        ...
        if (ssl->session->peer_cert->raw.len != ssl->session_negotiate->peer_cert->raw.len ||
            memcmp(
                ssl->session->peer_cert->raw.p, ssl->session_negotiate->peer_cert->raw.p,
...
int mbedtls_ssl_parse_certificate(mbedtls_ssl_context *ssl)
{
    ...
    const int authmode =
        ssl->handshake->sni_authmode != MBEDTLS_SSL_VERIFY_UNSET ? ssl->handshake->sni_authmode : ssl->conf->authmode;
#else
...
crt_verify:
    ...
    if (authmode != MBEDTLS_SSL_VERIFY_NONE)
    {
        ...
        ret = mbedtls_x509_crt_verify_restartable(
            ssl->session_negotiate->peer_cert, ca_chain, ca_crl, ssl->conf->cert_profile, ssl->hostname,
            &ssl->session_negotiate->verify_result, ssl->conf->f_vrfy, ssl->conf->p_vrfy, rs_ctx);
        ...
        if (mbedtls_ssl_check_cert_usage(
                ssl->session_negotiate->peer_cert, ciphersuite_info, !ssl->conf->endpoint,
        ...
        if (authmode == MBEDTLS_SSL_VERIFY_OPTIONAL &&
            (ret == MBEDTLS_ERR_X509_CERT_VERIFY_FAILED || ret == MBEDTLS_ERR_SSL_BAD_HS_CERTIFICATE))
        {
            ret = 0;
        }
...
static int
ssl_cookie_write_dummy(void *ctx, unsigned char **p, unsigned char *end, const unsigned char *cli_id, size_t cli_id_len)
{
    ((void)ctx);
    ((void)p);
    ((void)end);
...
static int ssl_cookie_check_dummy(
    void *ctx, const unsigned char *cookie, size_t cookie_len, const unsigned char *cli_id, size_t cli_id_len)
{
    ((void)ctx);
    ((void)cookie);
    ((void)cookie_len);
...
void mbedtls_ssl_conf_verify(
    mbedtls_ssl_config *conf, int (*f_vrfy)(void *, mbedtls_x509_crt *, int, uint32_t *), void *p_vrfy)
{
    conf->f_vrfy = f_vrfy;
    conf->p_vrfy = p_vrfy;
}
...
void mbedtls_ssl_conf_dbg(
    mbedtls_ssl_config *conf, void (*f_dbg)(void *, int, const char *, int, const char *), void *p_dbg)
{
    conf->f_dbg = f_dbg;
    conf->p_dbg = p_dbg;
}
...
void mbedtls_ssl_set_timer_cb(
    mbedtls_ssl_context *ssl, void *p_timer, mbedtls_ssl_set_timer_t *f_set_timer, mbedtls_ssl_get_timer_t *f_get_timer)
{
    ssl->p_timer = p_timer;
...
int mbedtls_ssl_set_hs_ecjpake_password(mbedtls_ssl_context *ssl, const unsigned char *pw, size_t pw_len)
{
    ...
    return (mbedtls_ecjpake_setup(
        &ssl->handshake->ecjpake_ctx, role, MBEDTLS_MD_SHA256, MBEDTLS_ECP_DP_SECP256R1, pw, pw_len));
}
...
void mbedtls_ssl_conf_psk_cb(
    mbedtls_ssl_config *conf, int (*f_psk)(void *, mbedtls_ssl_context *, const unsigned char *, size_t), void *p_psk)
{
    conf->f_psk = f_psk;
    conf->p_psk = p_psk;
}
...
int mbedtls_ssl_conf_dh_param_bin(
    mbedtls_ssl_config *conf, const unsigned char *dhm_P, size_t P_len, const unsigned char *dhm_G, size_t G_len)
{
    int ret;
...
void mbedtls_ssl_conf_sni(
    mbedtls_ssl_config *conf, int (*f_sni)(void *, mbedtls_ssl_context *, const unsigned char *, size_t), void *p_sni)
{
    conf->f_sni = f_sni;
    conf->p_sni = p_sni;
}
...
#if defined(MBEDTLS_SSL_EXPORT_KEYS)
void mbedtls_ssl_conf_export_keys_cb(
    mbedtls_ssl_config *conf, mbedtls_ssl_export_keys_t *f_export_keys, void *p_export_keys)
{
    ...
    while (ssl->in_offt == NULL)
    {
        ...
        if (ssl->in_msgtype == MBEDTLS_SSL_MSG_HANDSHAKE)
        {
...
#endif /* MBEDTLS_SSL_RENEGOTIATION */
            {
                ...
                {
                    if ((ret = mbedtls_ssl_send_alert_message(
                             ssl, MBEDTLS_SSL_ALERT_LEVEL_WARNING, MBEDTLS_SSL_ALERT_MSG_NO_RENEGOTIATION)) != 0)
                    {
                        return (ret);
                    }
                }
...
static int ssl_write_split(mbedtls_ssl_context *ssl, const unsigned char *buf, size_t len)
{
    ...
    if (ssl->conf->cbc_record_splitting == MBEDTLS_SSL_CBC_RECORD_SPLITTING_DISABLED || len <= 1 ||
...
int mbedtls_ssl_close_notify(mbedtls_ssl_context *ssl)
{
    ...
    {
        if ((ret = mbedtls_ssl_send_alert_message(
                 ssl, MBEDTLS_SSL_ALERT_LEVEL_WARNING, MBEDTLS_SSL_ALERT_MSG_CLOSE_NOTIFY)) != 0)
        {
...
static int ssl_preset_suiteb_ciphersuites[] = {
    MBEDTLS_TLS_ECDHE_ECDSA_WITH_AES_128_GCM_SHA256, MBEDTLS_TLS_ECDHE_ECDSA_WITH_AES_256_GCM_SHA384, 0};
...
int mbedtls_ssl_check_cert_usage(
    const mbedtls_x509_crt *cert, const mbedtls_ssl_ciphersuite_t *ciphersuite, int cert_endpoint, uint32_t *flags)
{
    int ret = 0;
...
int mbedtls_ssl_get_key_exchange_md_ssl_tls(
    mbedtls_ssl_context *ssl, unsigned char *output, unsigned char *data, size_t data_len)
{
    int ret = 0;
