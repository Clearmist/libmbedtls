...
static int
ssl_write_renegotiation_ext(mbedtls_ssl_context *ssl, unsigned char *buf, const unsigned char *end, size_t *olen)
{
    unsigned char *p = buf;
...
static int
ssl_write_signature_algorithms_ext(mbedtls_ssl_context *ssl, unsigned char *buf, const unsigned char *end, size_t *olen)
{
    unsigned char *p = buf;
...
static int ssl_write_supported_elliptic_curves_ext(
    mbedtls_ssl_context *ssl, unsigned char *buf, const unsigned char *end, size_t *olen)
{
    unsigned char *p = buf;
...
static int ssl_write_supported_point_formats_ext(
    mbedtls_ssl_context *ssl, unsigned char *buf, const unsigned char *end, size_t *olen)
{
    unsigned char *p = buf;
...
static int
ssl_write_ecjpake_kkpp_ext(mbedtls_ssl_context *ssl, unsigned char *buf, const unsigned char *end, size_t *olen)
{
    int ret;
    unsigned char *p = buf;
    size_t kkpp_len;
...
static int
ssl_write_max_fragment_length_ext(mbedtls_ssl_context *ssl, unsigned char *buf, const unsigned char *end, size_t *olen)
{
    unsigned char *p = buf;
...
static int
ssl_write_truncated_hmac_ext(mbedtls_ssl_context *ssl, unsigned char *buf, const unsigned char *end, size_t *olen)
{
    unsigned char *p = buf;
...
static int
ssl_write_encrypt_then_mac_ext(mbedtls_ssl_context *ssl, unsigned char *buf, const unsigned char *end, size_t *olen)
{
    unsigned char *p = buf;
...
static int
ssl_write_extended_ms_ext(mbedtls_ssl_context *ssl, unsigned char *buf, const unsigned char *end, size_t *olen)
{
    unsigned char *p = buf;
...
#if defined(MBEDTLS_SSL_SESSION_TICKETS)
static int
ssl_write_session_ticket_ext(mbedtls_ssl_context *ssl, unsigned char *buf, const unsigned char *end, size_t *olen)
{
    unsigned char *p = buf;
...
static int ssl_validate_ciphersuite(
    const mbedtls_ssl_ciphersuite_t *suite_info, const mbedtls_ssl_context *ssl, int min_minor_ver, int max_minor_ver)
{
    (void)ssl;
    if (suite_info == NULL)
        return (1);
...
static int ssl_write_client_hello(mbedtls_ssl_context *ssl)
{
    ...
    if (ssl->conf->max_major_ver == 0)
    {
        MBEDTLS_SSL_DEBUG_MSG(
            1, ("configured max major version is invalid, consider using mbedtls_ssl_config_defaults()"));
    ...
    *p++ = (unsigned char)n;
    ...
    if (ssl->conf->transport == MBEDTLS_SSL_TRANSPORT_DATAGRAM)
    {
        ...
        else
        {
            MBEDTLS_SSL_DEBUG_BUF(
                3, "client hello, cookie", ssl->handshake->verify_cookie, ssl->handshake->verify_cookie_len);
    ...
    for (i = 0; ciphersuites[i] != 0; i++)
    {
        ...
        if (ssl_validate_ciphersuite(ciphersuite_info, ssl, ssl->conf->min_minor_ver, ssl->conf->max_minor_ver) != 0)
            continue;
    ...
    *q++ = (unsigned char)(n << 1);
    ...
    if (offer_compress)
    {
        ...
        MBEDTLS_SSL_DEBUG_MSG(
            3, ("client hello, compress alg.: %d %d", MBEDTLS_SSL_COMPRESS_DEFLATE, MBEDTLS_SSL_COMPRESS_NULL));
...
#if defined(MBEDTLS_SSL_TRUNCATED_HMAC)
    if ((ret = ssl_write_truncated_hmac_ext(ssl, p + 2 + ext_len, end, &olen)) != 0)
    {
...
{
    if (ssl->conf->encrypt_then_mac == MBEDTLS_SSL_ETM_DISABLED || ssl->minor_ver == MBEDTLS_SSL_MINOR_VERSION_0 ||
        len != 0)
    {
...
{
    if (ssl->conf->extended_ms == MBEDTLS_SSL_EXTENDED_MS_DISABLED || ssl->minor_ver == MBEDTLS_SSL_MINOR_VERSION_0 ||
        len != 0)
    {
...
static int ssl_parse_server_hello(mbedtls_ssl_context *ssl)
{
    ...
    {
        MBEDTLS_SSL_DEBUG_MSG(
            1, ("server version out of bounds -  min: [%d:%d], server: [%d:%d], max: [%d:%d]", ssl->conf->min_major_ver,
                ssl->conf->min_minor_ver, ssl->major_ver, ssl->minor_ver, ssl->conf->max_major_ver,
    ...
    if (suite_info->key_exchange == MBEDTLS_KEY_EXCHANGE_ECDHE_ECDSA && ssl->minor_ver == MBEDTLS_SSL_MINOR_VERSION_3)
    {
...
static int ssl_write_encrypted_pms(mbedtls_ssl_context *ssl, size_t offset, size_t *olen, size_t pms_offset)
{
    ...
    if ((ret = mbedtls_pk_encrypt(
             &ssl->session_negotiate->peer_cert->pk, p, ssl->handshake->pmslen, ssl->out_msg + offset + len_bytes, olen,
             MBEDTLS_SSL_OUT_CONTENT_LEN - offset - len_bytes, ssl->conf->f_rng, ssl->conf->p_rng)) != 0)
    {
...
static int ssl_parse_signature_algorithm(
...
start_processing:
    ...
    if (mbedtls_ssl_ciphersuite_uses_server_signature(ciphersuite_info))
    {
        ...
        if (ssl->minor_ver == MBEDTLS_SSL_MINOR_VERSION_3)
        {
            if (ssl_parse_signature_algorithm(ssl, &p, end, &md_alg, &pk_alg) != 0)
            {
                ...
                mbedtls_ssl_send_alert_message(
                    ssl, MBEDTLS_SSL_ALERT_LEVEL_FATAL, MBEDTLS_SSL_ALERT_MSG_ILLEGAL_PARAMETER);
                return (MBEDTLS_ERR_SSL_BAD_HS_SERVER_KEY_EXCHANGE);
            }
        ...
        {
            hashlen = 36;
            ret = mbedtls_ssl_get_key_exchange_md_ssl_tls(ssl, hash, params, params_len);
            if (ret != 0)
                return (ret);
        }
        else
        ...
        if ((ret = mbedtls_pk_verify_restartable(
                 &ssl->session_negotiate->peer_cert->pk, md_alg, hash, hashlen, p, sig_len, rs_ctx)) != 0)
        {
#if defined(MBEDTLS_SSL__ECP_RESTARTABLE)
            if (ret != MBEDTLS_ERR_ECP_IN_PROGRESS)
#endif
...
static int ssl_write_client_key_exchange(mbedtls_ssl_context *ssl)
{
    ...
    if (ciphersuite_info->key_exchange == MBEDTLS_KEY_EXCHANGE_DHE_RSA)
    {
        ...
        ret = mbedtls_dhm_make_public(
            &ssl->handshake->dhm_ctx, (int)mbedtls_mpi_size(&ssl->handshake->dhm_ctx.P), &ssl->out_msg[i], n,
            ...
            ciphersuite_info->key_exchange == MBEDTLS_KEY_EXCHANGE_ECDH_ECDSA)
    {
        ...
        ret = mbedtls_ecdh_make_public(
            &ssl->handshake->ecdh_ctx, &n, &ssl->out_msg[i], 1000, ssl->conf->f_rng, ssl->conf->p_rng);
        if (ret != 0)
        {
        ...
        if (mbedtls_ssl_ciphersuite_uses_psk(ciphersuite_info))
    {
            ...
            if (ciphersuite_info->key_exchange == MBEDTLS_KEY_EXCHANGE_ECDHE_PSK)
        {
             ...
             */
            ret = mbedtls_ecdh_make_public(
                &ssl->handshake->ecdh_ctx, &n, &ssl->out_msg[i], MBEDTLS_SSL_OUT_CONTENT_LEN - i, ssl->conf->f_rng,
                ssl->conf->p_rng);
            if (ret != 0)
            {
                MBEDTLS_SSL_DEBUG_RET(1, "mbedtls_ecdh_make_public", ret);
        ...
        if (ciphersuite_info->key_exchange == MBEDTLS_KEY_EXCHANGE_ECJPAKE)
    {
        ...
        ret = mbedtls_ecjpake_write_round_two(
            &ssl->handshake->ecjpake_ctx, ssl->out_msg + i, MBEDTLS_SSL_OUT_CONTENT_LEN - i, &n, ssl->conf->f_rng,
            ssl->conf->p_rng);
        if (ret != 0)
        {
...
sign:
    ...
    if ((ret = mbedtls_pk_sign_restartable(
             mbedtls_ssl_own_key(ssl), md_alg, hash_start, hashlen, ssl->out_msg + 6 + offset, &n, ssl->conf->f_rng,
