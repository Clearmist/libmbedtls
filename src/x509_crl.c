...
int mbedtls_x509_crl_parse_der(mbedtls_x509_crl *chain, const unsigned char *buf, size_t buflen)
{
    ...
    {
        if (ret != (MBEDTLS_ERR_X509_INVALID_DATE + MBEDTLS_ERR_ASN1_UNEXPECTED_TAG) &&
            ret != (MBEDTLS_ERR_X509_INVALID_DATE + MBEDTLS_ERR_ASN1_OUT_OF_DATA))
        {
            mbedtls_x509_crl_free(crl);
            return (ret);
        }
    ...
    do
    {
        ...
        if (buflen == 0 || buf[buflen - 1] != '\0')
            ...
            ret = mbedtls_pem_read_buffer(
                &pem, "-----BEGIN X509 CRL-----", "-----END X509 CRL-----", buf, NULL, 0, &use_len);
...
int mbedtls_x509_crl_info(char *buf, size_t size, const char *prefix, const mbedtls_x509_crl *crl)
{
    ...
    ret = mbedtls_snprintf(
        ...
        prefix, crl->this_update.year, crl->this_update.mon, crl->this_update.day, crl->this_update.hour,
    ...
    ret = mbedtls_snprintf(
        ...
        "\n%snext update   : "
        "%04d-%02d-%02d %02d:%02d:%02d",
        prefix, crl->next_update.year, crl->next_update.mon, crl->next_update.day, crl->next_update.hour,
